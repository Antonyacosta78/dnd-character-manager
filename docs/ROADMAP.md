...
Current implementation note: backend-dependent auth, character-list, rules-catalog/import, and related operational command features were intentionally rolled back from the active repo in Step 7. The phase descriptions below remain planning targets for future reimplementation and should not be read as currently delivered functionality.
...
This phase is currently rolled back in the active repo baseline and remains a future reimplementation target.
